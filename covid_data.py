...
#########################################################################################
st.write('# Covid Cases Dashboard')
st.markdown(''' This is the dashboard showing the *Covid cases * of different type of :Countries:''')

DATA_URL = ('covid.csv')
df = pd.read_csv('covid.csv')

with st.form('line_chart'):
    selected_country = st.selectbox(label='Country', options=df['Country'].unique())
    submitted = st.form_submit_button('Submit')
    if submitted:
        filtered_avocado = df[df['Country'] == selected_country]
        line_fig = px.line(filtered_avocado,
                           x='date', y='total_cases',
                           color='total_cases',
                           title=f'Total_Cases in {selected_country}')
